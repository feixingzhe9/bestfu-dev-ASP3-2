...
{
    {0x01, LEVEL_1, Set_SwActSwitch_Attr        ,Get_SwActSwitch_Attr        ,1},
    {0x02, LEVEL_0, Set_SWActionTimer_Attr      ,NULL                        ,1},
    {0x08, LEVEL_0, Set_SWPowerOnStatus_Attr    ,Get_SWPowerOnStatus_Attr    ,0},
    {0x0a, LEVEL_0, Set_SW_ZeroCross_Attr_Enable,Get_SW_ZeroCross_Attr_Enable,0},     /**> 测试接口 **/
    {0x0b, LEVEL_0, NULL                        ,Get_SW_ZeroCross_Attr       ,0},     /**> 测试接口 **/
    {0x80, LEVEL_0, Set_SWActReversal_Attr      ,NULL                        ,1}
};

/*******************************************************************************
函 数 名:  Switch_UnitAttrNum
功能说明:  计算开关单元属性数量
参    数:  无
返 回 值:  无
*******************************************************************************/
u8 Switch_UnitAttrNum(void)
{
	return BF_DIM(SwitchAttrTable);
}
...
{
	u8 ucUNIT = unitID - 1;
	Sw_Act_Base_t* pSw_Act_Base = &Switch_Actuator.Sw_Act_Base[ucUNIT];

	pSw_Act_Base->PowerOnState = 0;
	EEPROM_Write(pSw_Act_Base->PowerOnEEPROMAddr, sizeof(pSw_Act_Base->PowerOnState), (u8*)&pSw_Act_Base->PowerOnState); 
}
...
MsgResult_t Get_SwActSwitch_Attr(UnitPara_t *pData, u8 *rlen, u8 *rpara)
{
	...
	*rlen = 1;
	*rpara = (0x00 == GPIOx_Get(pSw_Act_Base->pControlPort->ControlPort, pSw_Act_Base->pControlPort->ControlPin))?(0x00):(0x01);
	return COMPLETE;
}
...
MsgResult_t Set_SWPowerOnStatus_Attr(UnitPara_t *pData)
{
	...
	if (1 == pData->len)	                                /**> 参数长度正确  **/
	{
		...
		{
			pSw_Act_Base->PowerOnState = pData->data[0];
			result = EEPROM_Write(pSw_Act_Base->PowerOnEEPROMAddr, sizeof(pSw_Act_Base->PowerOnState), (u8*)&pSw_Act_Base->PowerOnState);
		}
	}		
	else
	{
...
{ 
	...
	result = EEPROM_Read(pSw_Act_Base->PowerOnEEPROMAddr, sizeof(pSw_Act_Base->PowerOnState), (u8*)&pSw_Act_Base->PowerOnState);
	if (COMPLETE == result)
	{
		*rlen = sizeof(pSw_Act_Base->PowerOnState);
		memcpy(rpara, &pSw_Act_Base->PowerOnState, *rlen);
	}
	return result;
}
...
MsgResult_t Set_SWActionTimer_Attr(UnitPara_t *pData)
{
	...
	pSw_Act_Base->SW_DelayClr();
	pSw_Act_Base->SwActTime.firstTime = ((*(u32*)&pData->data[1])) * 1000;
	pSw_Act_Base->SwActTime.secondTime= ((*(u32*)&pData->data[5])) * 1000;
	pSw_Act_Base->SwActTime.cycleFlag = pData->data[9];
	pSw_Act_Base->SwActTime.fun       = (pData->data[0] == 0 ? (pSw_Act_Base->SW_OFF) : (pSw_Act_Base->SW_ON));
...
MsgResult_t Set_SW_ZeroCross_Attr_Enable(UnitPara_t *pData)
{
	...
	pSw_Act_Base->SW_CrossZero.CheckFlagON = pData->data[0];
	pSw_Act_Base->SW_CrossZero.CheckFlagOFF= pData->data[1];
	return COMPLETE;
}
...
MsgResult_t Get_SW_ZeroCross_Attr_Enable(UnitPara_t *pData, u8 *rlen, u8 *rpara)
{
	...
	*rlen  		= 2;
	*rpara 		= pSw_Act_Base->SW_CrossZero.CheckFlagON;
	*(rpara+1)	= pSw_Act_Base->SW_CrossZero.CheckFlagOFF;
	return COMPLETE;
}
...
MsgResult_t Get_SW_ZeroCross_Attr(UnitPara_t *pData, u8 *rlen, u8 *rpara)
{
	...
	(*(u16*)(rpara))    = pSw_Act_Base->SW_CrossZero.CrossZeroTimeON;         /**< 继电器闭合时间 */
	(*(u16*)(rpara+2))  = pSw_Act_Base->SW_CrossZero.CrossZeroTimeOFF;        /**< 继电器断开时间 */  
	*rlen = 4;	
	return  COMPLETE;
}
...
void SWActInstruct(void)
{
	...
	LED_OPT[2].LED_OFF();
	Thread_Logout(Switch_Actuator.Sw_Act_Base[0].SW_OFF);
	Thread_Logout(Switch_Actuator.Sw_Act_Base[0].SW_ON);
	Thread_Logout(Switch_Actuator.Sw_Act_Base[1].SW_OFF);
	Thread_Logout(Switch_Actuator.Sw_Act_Base[1].SW_ON);
